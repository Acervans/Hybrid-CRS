...
EXPERT_CUTOFF = (
    1000  # Number of items from expert model (leeway before retraining again)
)
...
2.  **Get Recommendations**: Once you have some preferences, and ONLY if the user wants, call the `get_recommendations` tool to fetch some recommendations.
    -   You must NEVER generate recommendations yourself, NEVER use placeholders.
    -   They will be displayed all at once.
...
class HybridCRSWorkflow(Workflow):
    ...
    def _get_tools(self) -> List[FunctionTool]:
        ...
        return [
            FunctionTool.from_defaults(
                ...
                description="Gets personalized recommendations based on the user's current profile.",
            ),
            FunctionTool.from_defaults(
                self.update_user_preferences,
                name="update_user_preferences",
                description="Updates the user's profile with a new contextual preference, with validation.",
            ),
            FunctionTool.from_defaults(
                self.add_user_item_feedback,
                name="add_user_item_feedback",
                description="Processes natural language feedback, updating the session profile and backend data sources.",
            ),
            FunctionTool.from_defaults(
                self.end_session, name="end_session", description="Ends the session."
            ),
        ]
    ...
    async def process_user_input(
        ...
        if recs_exps := await ctx.store.get("last_recommendations", None):
            ...
            completion_stream = await llm.astream_complete(
        ...
        if await ctx.store.get("end_session", False):
            ...
            profile: UserProfile = await ctx.store.get("profile")
            return StopEvent(result=profile.model_dump_json())
    ...
    async def process_json_feedback(
        ...
        # Reply after processing feedback
        memory: ChatMemoryBuffer = await ctx.store.get("memory")
        completion_stream = await llm.astream_complete(
